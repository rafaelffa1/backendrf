...
const { findConnections, findConnectionsRestaurant, sendMessage } = require('./websocket');
...
router.get("/consultTransactions/pagseguro/:email/:token", (req, res) => {
  ...
  let consultTransactions = request.get({
    url: `https://ws.sandbox.pagseguro.uol.com.br/v2/transactions?email=${req.params.email}&token=${req.params.token}&initialDate=${rangeTime.toISOString()}&finalDate=${currentTime.toISOString()}`
  },
    function (err, response, body) {
      if (err) {
        cb(err)
...
router.post("/firebase/token", (req, res) => {
  admin.messaging().sendMulticast({
    ...
    notification: {
      ...
      imageUrl: 'https://i.pinimg.com/originals/71/72/16/7172161b580470deb78078669236d2c1.jpg',
    },
  });
...
router.post("/login/mobile", (req, res) => {
  ...
  UsuarioController.loginUsuarioMobile(req.body.celular, req.body.senha, callback);
});
...
router.post('/usuario/redefinir', (req, res) => {
  function callback(email, senha) {
    ...
    transporter.sendMail(mailOptions, function (error, info) {
      ...
      if (error) {
        console.log(error);
        res.json({ 'text': 'Não foi possivel enviar o email, por favor tente novamente mais tarde.', status: false })
      } else {
        console.log('Email enviado: ' + info.response);
        res.json({ 'text': 'Verifique sua caixa de spam e lixeira.', "status": true })
      }
    });
...
router.get("/rifa/usuario/:id/pagination/:page", (req, res) => {
  ...
  RifaController.selectRifasUsuarioPaginado(callback, req.params.id, req.params.page);
});
...
router.get("/rifa/status/:status/pagination/:page", (req, res) => {
  ...
  RifaController.selectRifaStatusPaginado(callback, req.params.status, req.params.page);
});
...
router.get("/rifa/categoria/:categoria/pagination/:page", (req, res) => {
  ...
  RifaController.selectRifaCategoriaPaginado(callback, req.params.categoria, req.params.page);
});
// ===================== Rifa ==========================================
