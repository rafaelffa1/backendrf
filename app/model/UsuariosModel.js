...
exports.insertUsuarios = function (data, senha, tipo, enderecoID, imagem) {
  db.run(
    ...
    [data.email, data.celular, senha, data.nome, data.sobrenome, data.cpf, data.rg, data.orgao_emissor, tipo, enderecoID, imagem, null, null, null],
    function (err) {
      if (err) {
...
exports.insertUsuariosMobile = function (data, callback) {
  db.run(
    ...
    [data.email, data.celular, data.senhaB, data.nome, null, null, null, null, 4, null, data.foto, null, data.socialID, data.tokenDevice],
    function (err) {
      if (err) {
        return console.log(err.message);
      } else {
...
exports.selectIdUsuario = function (callback, usuarioID) {
  db.serialize(function () {
    db.all(`SELECT * FROM usuarios WHERE ID == ${usuarioID}`, function (err, allRows) {
      if (err != null) {
        console.log(err);
      }
      callback(allRows);
...
exports.loginUsuarioMobile = function (celular, callback) {
  db.serialize(function () {
    db.all(`SELECT * FROM usuarios WHERE celular == '${celular}'`, function (err, allRows) {
      if (err != null) {
        console.log(err);
      }
      callback(allRows[0]);
...
exports.validarUsuarioCelular = function (celular, callback) {
  db.serialize(function () {
    db.all(`SELECT ID, email, nome, celular FROM usuarios WHERE celular == '${celular}' AND tipo == 4`, function (err, allRows) {
      if (err != null) {
        console.log(err);
      }
      callback(allRows[0]);
...
exports.validarUsuarioEmail = function (email, callback) {
  db.serialize(function () {
    db.all(`SELECT ID, email, nome, celular FROM usuarios WHERE email == '${email}' AND tipo == 4`, function (err, allRows) {
      if (err != null) {
        console.log(err);
      }
      callback(allRows[0]);
...
exports.validarUsuarioSocialID = function (socialID, callback) {
  db.serialize(function () {
    db.all(`SELECT ID, email, nome, celular, imagem FROM usuarios WHERE socialID == '${socialID}' AND tipo == 4`, function (err, allRows) {
      if (err != null) {
        console.log(err);
      }
      callback(allRows);
...
exports.selectUsuarioAdmin = function (callback, token) {
  db.serialize(function () {
    db.all(`SELECT ID, email, nome, sobrenome, imagem, tipo FROM usuarios WHERE usuarios.tokenLogin = '${token}'`, function (err, allRows) {
      if (allRows.length === 0) {
